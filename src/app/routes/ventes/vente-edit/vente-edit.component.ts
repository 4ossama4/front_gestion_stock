...
@Component({
  ...
  styleUrls: ['./vente-edit.component.less'],
})
export class venteEditComponent implements OnInit {
    ...
    this.ventesService.getVenteById(this.venteId).subscribe(
      (response: any) => {
        ...
        this.venteForm.patchValue({
          date: response.data.date_vente,
          commerciale: response.data.commercial,
          reference: response.data.reference_vente,
          prix_vente_ttc: response.data.prix_vente_ttc,
          prix_vente_ttc_with_remise: response.data.prix_vente_ttc_with_remise,
          commercial_total: response.data.commercial_profit,
          prix_vente_ht: response.data.prix_vente_ht,
          net_vente: response.data.net_vente,
          is_paid: response.data.is_paid == 0 ? false : true,
        });
        ...
        response.data.lignes_vente.forEach((ligne_vente: any) => {
          ...
          control.push(
            this.patchValues(
              ...
              ligne_vente.prix_vente,
              ligne_vente.remise,
              ligne_vente.prix_vente_avec_remise,
              ligne_vente.total,
              ligne_vente.total_with_remise,
              ligne_vente.quantite_facture,
              ligne_vente.quantite_restante,
            ),
          );
        });
  ...
  saveVente() {
    ...
    if (this.venteForm.valid && !this.clientInvalid && this.venteForm.value.lignes_vente.length > 0) {
      this.ventesService.update(this.venteForm.value).subscribe(
        (reponse) => {
