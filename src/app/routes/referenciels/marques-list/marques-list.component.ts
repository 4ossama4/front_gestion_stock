...
@Component({
  ...
  styleUrls: ['./marques-list.component.less'],
})
export class marquesListComponent implements OnInit {
  ...
  public confirmationDelete(item: any) {
    this.stockService.deleteMarque(item.id).subscribe(
      (response) => {
  ...
  public submitForm() {
    ...
    if (this.refForm.valid) {
      if (!this.modeEdit) {
        this.stockService.storeMarque({ label: this.refForm.value.label }).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getMarques();
        ...
        this.stockService.updateMarque(this.refForm.value).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getMarques();
            this.notificationService.createNotification('success', 'Marque a été modifier avec succes', null);
          },
          (error) => {
