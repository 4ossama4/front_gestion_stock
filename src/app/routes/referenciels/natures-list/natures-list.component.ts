...
@Component({
  ...
  styleUrls: ['./natures-list.component.less'],
})
export class naturesListComponent implements OnInit {
  ...
  public confirmationDelete(item: any) {
    this.stockService.deleteNature(item.id).subscribe(
      (response) => {
  ...
  public getNatures() {
    ...
    this.stockService.getNatures().subscribe(
      (response: any) => {
        this.data = response;
  ...
  public submitForm() {
    ...
    if (this.refForm.valid) {
      if (!this.modeEdit) {
        this.stockService.storeNature({ label: this.refForm.value.label }).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getNatures();
        ...
        this.stockService.updateNature(this.refForm.value).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getNatures();
