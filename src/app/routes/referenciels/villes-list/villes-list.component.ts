...
@Component({
  ...
  styleUrls: ['./villes-list.component.less'],
})
export class villesListComponent implements OnInit {
  ...
  public confirmationDelete(item: any) {
    this.stockService.deleteVille(item.id).subscribe(
      (response) => {
  ...
  public getVilles() {
    ...
    this.stockService.getVilles().subscribe(
      (response: any) => {
        this.data = response;
  ...
  public submitForm() {
    ...
    if (this.refForm.valid) {
      if (!this.modeEdit) {
        this.stockService.storeVille({ label: this.refForm.value.label }).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getVilles();
        ...
        this.stockService.updateVille(this.refForm.value).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.getVilles();
            this.notificationService.createNotification('success', 'Ville a été modifier avec succes', null);
          },
          (error) => {
