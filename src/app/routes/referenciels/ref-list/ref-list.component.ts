...
@Component({
  ...
  styleUrls: ['./ref-list.component.less'],
})
export class refListComponent implements OnInit {
  ...
  public confirmationDelete(item: any) {
    this.stockService.deleteReference(item.id).subscribe(
      (response) => {
  ...
  public chercheRefOriginal(event: any) {
    ...
    this.stockService.chercheRefOriginal(label).subscribe(
      (response: any) => {
        this.data = response;
  ...
  public submitForm() {
    ...
    if (this.refForm.valid) {
      if (!this.modeEdit) {
        this.stockService.storeReference({ label: this.refForm.value.label }).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.chercheRefOriginal(' ');
        ...
        this.stockService.updateReference(this.refForm.value).subscribe(
          (response: any) => {
            this.refForm.reset();
            this.modeEdit = false;
            this.chercheRefOriginal(' ');
            this.notificationService.createNotification('success', 'Reference a été modifier avec succes', null);
          },
          (error) => {
